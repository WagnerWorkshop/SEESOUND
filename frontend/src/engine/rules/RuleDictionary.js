...
const _INPUT_RANGES = Object.freeze({
    ...
    objectShapeSine: [0, 1],
    objectShapeTriangle: [0, 1],
    objectShapeSawtooth: [0, 1],
    objectShapeSquare: [0, 1],
    objectShapeNoise: [0, 1],
    objectShapePinkNoise: [0, 1],
    objectShapeTransient: [0, 1],
    objectShapePad: [0, 1],
...
const _INPUT_MODES = {
    ...
    // Per-object variables (cloud mode only)
    objectFundamentalHz: ['cloud'],
    objectShapeSine: ['cloud'],
    objectShapeTriangle: ['cloud'],
    objectShapeSawtooth: ['cloud'],
    objectShapeSquare: ['cloud'],
    objectShapeNoise: ['cloud'],
    objectShapePinkNoise: ['cloud'],
    objectShapeTransient: ['cloud'],
    objectShapePad: ['cloud'],
    objectShapeBuzzy: ['cloud'],
...
const _outputEntries = [
    ...
    { id: 'gradientU', type: 'number', range: [0, 1], targets: ['spawnedParticles', 'allParticles', 'lines', 'background'], modes: ['particle', 'cloud'] },
...
export function sanitizeRuleBlocks(ruleBlocks, dictionaries = { input: inputDictionary, output: outputDictionary }) {
    ...
    input.forEach((rule, index) => {
        const coerced = {
            ...
            layerId: typeof rule?.layerId === 'string' ? rule.layerId : '',
            layerName: typeof rule?.layerName === 'string' ? rule.layerName : '',
