...
export function useWebSocket(onMessage) {
    ...
    const connect = useCallback(() => {
        ...
        socket.onopen = () => {
            ...
            if (outboundQueue.current.length > 0) {
                for (const payload of outboundQueue.current) {
                    try {
    ...
    const sendMessage = useCallback((payload) => {
        if (ws.current?.readyState === WebSocket.OPEN) {
            ...
            if (outboundQueue.current.length > 200) {
                outboundQueue.current = outboundQueue.current.slice(-200)
            }
            console.warn('[WS] Queued message — socket is not open')
        }
    }, [])
